...
- **Every drop-in that introduces hotkeys must add its control lines to its
  ...
  `register_help_entries`) so they appear in the `H` help overlay
  automatically.  This is the single source of truth for all key bindings;
  do not document keys anywhere else (no separate README sections, no
  docstrings listing keys).
