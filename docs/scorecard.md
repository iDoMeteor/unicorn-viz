...
| **OS support** | **B−** | B+ | B+ | B+ | B+ | **A** | C | **Re-graded 2026-08-07** after the owner corrected the platform priority: unicorn-viz is **Linux-first**, Windows second, macOS third. The old C+ carried a coherence penalty for declaring Windows primary while shipping it worst; that penalty is gone, and what remains is graded purely on delivered breadth.<br>Us: **Linux is excellent** — Wayland-native, PipeWire, the daily development and performance platform. **Windows runs** but the dependency stack still wants a MinGW toolchain and a hand-patched `python-rtmidi` on current Python, the installer is ★1 of 5, and this week's DPI/multi-head fixes are *not yet hardware-verified*. **macOS: planned, nothing shipped.** One platform excellent, one rough, one absent.<br>**The strategic point this row hides:** of the five paid products here, **not one runs on Linux at all.** For a Linux user the entire commercial field is empty and the only alternatives are projectM and us. Being best-in-class on the platform nobody else serves is worth more than being fifth-best on Windows — the grade measures breadth, but the *position* is stronger than B− suggests.<br>Resolume / Synesthesia / Magic / TouchDesigner: Windows + macOS with polished installers; **no Linux** on any of them (TouchDesigner explicitly Windows/macOS only; Linux via Wine is unofficial and unsupported).<br>projectM: **the breadth winner** — Linux, Windows, macOS, Android, and OpenGL-ES embedded targets.<br>VDMX: **macOS only.** |
...
| **Video output interop** (NDI / Spout / Syphon / PipeWire) | **B** | **A** | A | B+ | A | — | **A** | **Was a D; now B as of video-out-01 0.2.0 (2026-08-07).** Us: a **v4l2loopback virtual camera** (OBS and any V4L2 consumer take it natively, no plugin) **plus a zero-copy PipeWire/DMA-BUF path** — the real Linux equivalent of Spout/Syphon, which no competitor here implements *because none of them runs on Linux*. Held to B rather than A on three honest counts: the PipeWire path is **built but unverified on hardware**, there is still **no NDI** so nothing crosses machines, and Windows/macOS users get nothing until Spout/Syphon land. Output is our own window(s), a recording file, or an RTMP stream. To get unicorn-viz into OBS today you screen-capture a window — which works, but costs a copy and hands the compositor a job it shouldn't have.<br>Resolume: Spout/Syphon/NDI in and out, the reference implementation.<br>Synesthesia: full Syphon/Spout/NDI — but **Pro tier ($399) only**.<br>Magic: Spout/Syphon output (*NDI unverified*).<br>TouchDesigner: everything, plus SDI/Blackmagic hardware I/O.<br>VDMX: Syphon is native to how Mac VJs work; excellent.<br>projectM: a rendering library — interop is whatever the host app provides.<br>**This is the highest-value single flip on the board.** Spout (Windows) and NDI would put us inside every OBS/Resolume rig in the world, and Windows is our declared primary target. |
...
|---|:-:|---|---|
| Projection mapping & media-server duty | — | Resolume Arena (the €500 premium over Avenue *is* this), TouchDesigner, VDMX, HeavyM, MadMapper | **Moved out of the scored table 2026-08-07 (owner decision).** Mapping is a different craft: mesh warping, masking, keystone correction and edge blending are **geometry correction against a physical site**, validated with projectors, ladders and a tape measure. Our creative work happens in *time*, against music. It is also untestable here — you cannot verify edge blending without two overlapping projectors and an irregular surface, the same validation trap that put DVS out of scope on the mixer. **The honest workflow answer if a venue ever asks:** run unicorn-viz into Resolume Arena over the interop path (see the video-output-interop plan) and let Arena do the geometry — which is exactly why that interop row matters more than this one ever did. |
...
- **The honest second gap is OS breadth — but the position is better than
  the grade.** We are Linux-first (corrected 2026-08-07), Windows second,
  macOS third, and only Linux is genuinely finished. projectM's A there is
  a standing rebuke: an LGPL project from 2003 runs cleanly on more
  platforms than we do. **The flip side is the more useful fact: not one of
  the five paid products in this table runs on Linux at all.** On our
  primary platform the commercial field is empty, so the competitive
  question there is not "are we as good as Resolume" but "are we better
  than projectM" — a very different, and much more winnable, fight.
...
- 2026-08-07 (same day, owner corrections): **projection mapping removed
  from the scored table** and expanded in non-compete — it is geometry
  correction against a physical site, unverifiable without projectors, and
  the honest answer is to feed Resolume Arena over the interop path rather
  than to build it. **OS support C+ → B−** after the owner corrected the
  platform priority to **Linux-first, Windows second, macOS third**: the old
  grade carried a coherence penalty for declaring Windows primary while
  shipping it worst, and that penalty no longer applies. Added the note that
  *no paid product in this table runs on Linux at all*, which reframes our
  primary platform as an open field rather than a gap. **Video interop
  planned** — see `docs/planning/video-output-interop-plan-2026-08-07.md`
  (PipeWire/DMA-BUF and v4l2loopback first, NDI next, Spout/Syphon with
  those platforms). Overall stays **B+**: the interop D is the anchor.
