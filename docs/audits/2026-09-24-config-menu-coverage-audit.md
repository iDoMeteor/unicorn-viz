...
| 7 | core 1.0.0-beta.178 | The effects seat's spec for the 21 construction keys + 3 VideoShowcase keys: **11 rows**, shown only when the effect is registered. Visuals › projectM: blend between presets, lock preset, start presets clean, opening preset, auto-exclude flat presets, flat-color grace time (NEXT LOAD: the effect config is rebuilt on each activation). Performance: preload images at startup (RESTART), preset warm-up frames (NEXT LOAD), libprojectM path and extra preset folders (RESTART), preset texture folders (NEXT LOAD). Stored as `config_overrides.effects.<Class>.<key>`; config.toml values migrate (a TOML folder list becomes the comma string). **No row, 13:** `SimShowcase.mix_time` / `camera_energy` (already live parameters, covered by batch 6), `random_zoom_min` ×6 (redundant after effects-rollercoast 0.5.1), `ProjectMEffect.fps_hint` (vestigial), `ImageShowcase.scale_when_framed` and the three `VideoShowcase` keys (no reader; not remapped to VideoClips on purpose) |
| 8 | core 1.0.0-beta.179, auto-vj-01 rc.153-154 | The Auto VJ seat's spec for the remaining `[auto_vj]` keys: hooks in rc.153 (their seat), **16 rows** in rc.154: on/off (live, remembered), engine and onset envelope (RESTART), HUD BPM smoothing on/off and time (Visuals › HUD), build/breakdown/climax snap + breakdown/climax phrase wait + climax phrase grid (Auto VJ - director), drop trigger / fastlane / sustain entry / fizzle floor / cruise post-FX slots (Auto VJ - USER mood; these apply only while the mood is USER, as before). Core: **Production HUD** (`hud_production_mode`, Visuals › HUD detail, live). **No row:** `mode_source_min_confidence_build` / `_rel` and `drop_cruise_min_confidence` (experiment flags, owner values equal code defaults; replays still reach them with `--override`) |
| 9 | core 1.0.0-beta.180, auto-vj-01 rc.155, streaming-01 0.8.1, video-out-01 0.7.1, chat-01 0.7.1, media-01 0.31.1, spotify-01 rc.12 | **Reorganization** (owner: Performance was a catch-all). Performance keeps cost knobs only. New **System** tab: display, audio engine, MIDI, folders (ANSI art, media), projectM paths, Spotify client ID. New **Streaming** tab: RTMP, video out, chat username. New **Auto VJ** tab (appears with the drop-in): every Auto VJ row, sectioned General / Detector / Director / USER mood / Recommender / HUD / Logs and training. All three new tabs follow the machine; a live row that left a profile tab keeps the active profile's saved value (carried over once) |
