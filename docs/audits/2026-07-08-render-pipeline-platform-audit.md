...
| 11 | Root-cause control-room/mixer showing black despite successful render+present (per items 1-2's own diagnostics) | High — causation proven by isolation test (0 GL errors without control-room/mixer vs. ~408k with, otherwise identical session) | Low | **Likely fixed, awaiting owner confirmation** (`5ea7ab5`). Root cause was not cross-window/Mutter-level corruption after all: `_read_streaming_frame()` only ever runs when a subsystem needs frame bytes (control-room), so it was simply never exercised in the isolation test. The GL error cascade started with a single `glReadBuffer(invalid buffer GL_COLOR_ATTACHMENT0)` (Mesa rejecting that enum against whatever framebuffer was actually bound), 3 lines before ~177k `glUseProgram(GL_INVALID_VALUE)` errors and cascading into ~60k `glBindVertexArray`/`glUniform` and ~54k `glBufferSubData` errors for the rest of the session — one bad read call appears to leave the whole GL context in a broken error state. `render_effects_browser_thumbnail()` already had to defensively `fbo.use()` + `finally: self._ctx.screen.use()` for exactly this reason (see its own docstring); `_read_streaming_frame()` never did the equivalent, so if any subsystem's `render_overlay()` hook left a different FBO bound, our read silently inherited it. Fixed by calling `source.use()` immediately before every read |
