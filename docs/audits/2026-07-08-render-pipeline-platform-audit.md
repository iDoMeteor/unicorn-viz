...
| 11 | Root-cause control-room/mixer showing black despite successful render+present (per items 1-2's own diagnostics) | Mutter/Wayland-geometry theory ruled out — same black result on GNOME Wayland, GNOME Classic, **and** MATE/X11 (owner tested all three, identical result including the GL_INVALID_VALUE PBO error on X11 too). Revised theory, medium-high confidence: driver-level GL state corruption from running a second SDL window (control-room/mixer, pure SDL_Renderer+PIL, zero direct GL calls) in the same process as an active moderngl context | Unknown — may require the debug handoff's "Path 4" architecture escalation (separate process) rather than an in-process fix | Open. Owner captured MESA_DEBUG=1/LIBGL_DEBUG=verbose output directly in a terminal (screenshot) showing a flood of real GL errors during a control-room+mixer session: repeated `GL_INVALID_VALUE in glBindVertexArray(non-gen name)`, `GL_INVALID_VALUE in glBufferSubData(offset 0 + size 18432 > buffer size 0)`, and `GL_INVALID_OPERATION in glUniform2f("tex0"@0 is sampler, not float)` — none of which control-room/mixer's own code could cause directly (confirmed: neither file makes any OpenGL call). Since these hit the *main audience* window's legitimate GL calls (uniform sets, VAO/buffer ops), and the same black result reproduces across three completely different desktop environments/compositors, this points back to the very first hypothesis from the original audit (§1): a second in-process SDL window can corrupt the primary GL context at the driver level, not just crash it. The earlier `rebind_main_gl_context()` fix (item 1) stopped the crash but evidently not this. Owner is running one more isolation test: same MESA_DEBUG capture with control-room/mixer never opened at all, to confirm whether the GL error flood is actually caused by their presence or is a pre-existing bug elsewhere in the effects pipeline |
