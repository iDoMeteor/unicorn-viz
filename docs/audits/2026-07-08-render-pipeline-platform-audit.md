...
| 9 | Root-cause why PBO buffer mapping fails ("cannot map the buffer") in the first place | Unknown — needs live Mesa/Iris Xe GL debugging under Wayland | Medium | Open, deferred — diagnostic tooling now in place (`e69347f`): `[logging] level = "DEBUG"` auto-sets `MESA_DEBUG`/`LIBGL_DEBUG`/`EGL_LOG_LEVEL`, and the failure log now includes which specific call failed, the raw `glGetError()` code, buffer size, canvas dimensions, mirror-mode state, and frames-succeeded-before-failure. Next actual repro just needs a DEBUG-level run with stderr redirected to a file (`... 2>&1 \| tee gl_debug.log`) sent back for review — see §6 for the fuller protocol including `apitrace` as the heavier fallback |
