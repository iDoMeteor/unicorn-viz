...
void main() {
    ...
    // Nebula background
    vec3 col = nebula(uv * 0.5, t);

    // Three parallax star layers: deep / mid / near
    col += starLayer(uv,              t,        42.0, 0.12, 0.91);
    col += starLayer(uv + 0.17,       t * 1.4,  72.0, 0.22, 0.88);
    col += starLayer(uv + 0.41,       t * 2.1,  110.0, 0.38, 0.84);

    // Warp streaks from beat
    col += warpStreaks(v_uv, t, iWarp);
