...
- **1.0.0-beta.180** — **The config menu is reorganized.** Performance now holds only the settings that cost frames. A new **System** tab has display, MIDI, the audio engine, folders and paths, and the Spotify client ID. A new **Streaming** tab has RTMP, video out and the chat username. Auto VJ gets **its own tab** with every Auto VJ setting in sections (General, Detector, Director, USER mood, Recommender, HUD, Logs and training). Your saved values come along.
...
- **1.0.0-beta.148** — Config editor: a UI scale (1.0 at 1080p, 1.8 at 4K) so the panel keeps its 1080p proportions on a 4K primary; the Hotkeys tab shows the MIDI notes bound to each action (read-only, from the active note map). Help overlay: the "Extra effects (no direct key)" list wraps to its pane (at most three lines, ellipsized) instead of running off the panel. Runtime-capability discovery gains a loaded-only mode (reads the drop-in module cache, imports nothing) exposed as `vj_api.dropin_capabilities()` and `subsystem_names()`; control-room-01 0.19.0 uses it so the LAYOUT page names loaded drop-ins that have no panel yet.
- **1.0.0-beta.147** — Tier B of the drop-in performance audit lands: banner-01 0.14.0 (drip, beat color), cta-01 0.10.0 (editor FX, drip, sparkles, density), midi-controllers-01 0.11.0 (LED refresh rate), effects-particles 0.10.0 (`particles`), effects-tech 0.11.0 (`march_steps`), effects-rollercoast 0.5.0 (`march_steps` on every ride), images-01 0.9.0 (`[images] max_image_edge`), unicorn-tears-01 rc.3 (`[dancing_unicorn] max_sparkles`). The unicorn-tears controller joins the config editor's contributor attribute list. A handoff doc for the VJ seat covers auto-vj-01's rows (docs/planning). sims-01 and video-clips-01 were reclassified: no running cost knob to expose.
- **1.0.0-beta.146** — **Config editor contributor convention 2.0.** Drop-ins are discovered through the subsystem registry (no more fixed list in core), a row can pick its own tab and carry the 2.0 presentation keys (kind, choices, label, hint, badge, section), live Performance rows are remembered in runtime state and replayed at startup, and `restart` rows lay their config values over the loaded config now and at the next launch. Enables the drop-in Performance rows landing alongside (see docs/audits/2026-09-16-dropin-performance-options.md).
- **1.0.0-beta.145** — **Now Playing banner gets a switch.** The track-change announcer had no key and no editor row: it was config-only, one `now_playing_banner` flag per source (mixer, Spotify, media player). `Shift+W` now mutes it across every source at once (`W` stays the platter), and both the banner and the Now Spinning platter appear as toggles under Announcements on the config editor's Visuals tab. Both choices are remembered in runtime state across runs.
...
- **1.0.0-beta.142** — Windows packs render every text surface with the
  ...
  operator window, help modal and profiles panel drew with PIL's tiny bitmap
  font on Windows).
- **1.0.0-beta.141** — Video decks audit fixes: a deck refused a source at the two-source cap is retried each frame and picks one up as soon as another deck frees it (it used to stay black for the rest of its track); the layer's shader compiles at construction, not on the first fade-in; an audio-only night costs a null check per frame; the open log line no longer prints a 0x0 size (the open is non-blocking — the first uploaded frame logs the real size); `DeckVideoSource.open_error` (videos-01 0.8.2) is honored with one WARNING and the deck treated as no-video.
- **1.0.0-beta.140** — Video decks: the layer targets the *audible* instant when the mixer publishes its output latency (`latency_s` in the deck-state payload) — the mixer's position is its write cursor, and on this rig the output buffer is 256 ms, so without this the picture led the sound by a quarter second. Separate texture-upload timing, an opt-in per-frame trace (`UNICORNVIZ_VIDEO_DECKS_TRACE=<file>`), and `tools/profiling/video_decks_report.py` for the part-B measurement table.
