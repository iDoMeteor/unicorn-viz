...
class BeatTracker:
    ...
    def update(
        ...
        if onsets:
            ...
            # No onset events; just advance envelope to current time
    ...
    def _advance_envelope(self, target_t: float) -> None:
        """Advance the envelope write pointer to target_t using zero-fill."""
        if self._last_t < 0:
            self._last_t = target_t
            return
        elapsed = max(0.0, target_t - self._last_t)
        if elapsed <= 0.0:
            return
        self._env_t_acc += elapsed
        step = 1.0 / _V2_ENV_RATE
        while self._env_t_acc >= step:
            self._env_t_acc -= step
            # No onset: fill with 0
    ...
    def _pulse_envelope(self, strength: float, t: float = 0.0) -> None:
        ...
        self._env_buf[self._env_write_idx] = max(
            self._env_buf[self._env_write_idx], s
        )
        # Advance past this sample so zero-fill can't clobber it
        self._env_write_idx = (self._env_write_idx + 1) % self._env_len
        if self._env_write_idx == 0:
            self._env_filled = True
        # Deduct the consumed slot from the accumulator to keep time in sync
        self._env_t_acc = max(0.0, self._env_t_acc - 1.0 / _V2_ENV_RATE)
